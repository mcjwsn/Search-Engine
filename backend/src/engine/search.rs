...
use sprs::CsVec;
use crate::matrix::{TfIdfMatrix, SingularValueDecomposition};
...
pub fn search_with_svd(query: &str, tfidf: &TfIdfMatrix, svd: &SingularValueDecomposition, k: usize) -> Vec<(usize, f64)> {
    // Create a low-rank approximation of the TF-IDF matrix
    let low_rank_matrix = tfidf.low_rank_approximation(svd);

    // Use the same tokenization and TF-IDF calculation as the original search
    let mut query_tf: HashMap<usize, usize> = HashMap::new();
    let mut total_terms = 0;
    ...
    // Calculate cosine similarity using the low-rank approximation matrix
    let mut similarities = Vec::new();
    for doc_idx in 0..low_rank_matrix.cols() {
        let doc_vec = low_rank_matrix.outer_view(doc_idx).unwrap();
        let sim = normalized_q.dot(&doc_vec).abs();  // |cos θ|
        similarities.push((doc_idx, sim));
    }
